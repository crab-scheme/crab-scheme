...
mTLS/QUIC run a real cluster **PKI** (cs-net's `dev-certs` feature): a
process-global self-signed **CA**, and a **per-node leaf cert** signed by it
(carrying the node name, serverAuth+clientAuth). Every node presents its own
CA-issued cert and verifies its peer's cert chains to the cluster CA root — real
mutual authentication of distinct identities, not a single shared cert. The CA
verification is tested both ways: `mtls_ca_accepts_peer_signed_by_cluster_ca`
and `mtls_ca_rejects_peer_signed_by_foreign_ca` (a cert from a different CA is
refused). The dev gap a production cluster closes: the CA key lives in-process
and is regenerated per run, and certs aren't loaded from disk. (Node identity
proper is still the cs-distrib `Hello` (NodeId).)
...
4. **Run over real transport** — done: ~~actor-driven in-process~~
   ...
   (`raft-net-tcp.scm`), ~~mutual TLS~~ (`raft-net-tls.scm`), ~~QUIC~~
   (`raft-net-quic.scm`), and ~~per-node certs from a cluster CA~~ (cs-net
   `dev-certs`). Remaining for production: load the CA + node certs **from disk /
   an external CA** instead of the in-process dev CA.
