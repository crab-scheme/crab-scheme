...
|---|---|---|
| 7.1 + 7.1.x | **Shipped.** Pair tombstone infrastructure (WeakValue type, car_weak/cdr_weak fields, ~250 reader sites migrated). Strong-count-guarded break invoked from b_set_car/b_set_cdr — demote fires for cycles with multiple persistent external anchors (count >= 5 threshold accounting for slot + caller args + VM dispatch temps); skips cycles whose only anchor is in the freshly-mutated subgraph (e.g. the metacircular `(set-car! env (cons name val))` pattern leaks rather than orphans the cons cell). `cycle_broken_count` distinguishes detection from successful break. 4 new regression tests in crates/cs-runtime/tests/cycle_break.rs. |
| 7.1.x.y | Precise per-tier baseline accounting for the temp-ref count, OR full Bacon-Rajan trial-deletion to pick a safe cycle edge agnostic to caller conventions. Currently the threshold-5 heuristic leaks cycles with a single external anchor. |
