...
| FR-5: continuation / closure cycle prevention | `requirements.md` | **Partial** — iter-6 identity-dedup (`CycleVisitor::visit_addr` on Frame/Env/Closure/VmClosure) bounds the detector. The `Weak<T>` back-edge refactor (iter 8) closes the remaining refcount-leak gap and is queued as follow-up. |
...
| 8 | `Frame.parent` / `Continuation` parent chain refactored to `Weak<Frame>`. Continuation captures keep the leaf strong; ancestors walked via `upgrade()`. | Cascades through ~30+ walker call sites that traverse the parent chain. Same refactor scope concern as iter 7.1. |
