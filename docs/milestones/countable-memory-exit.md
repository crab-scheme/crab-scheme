...
|---|---|---|
| 7.1 | Strong/Weak storage tombstone infrastructure on `Pair`. WeakValue type, `Pair::{car_weak, cdr_weak}` tombstone fields, accessor methods, ~250 reader sites migrated workspace-wide. **Naive break demonstrated unsafe** — demoting the freshly-mutated slot to Weak orphans the value when the slot was its only strong holder (the `(set-car! env (cons name val))` closures-over-env pattern, broke metacircular). Currently the break_at action records to the counter but does NOT structurally break; cycles still leak refcount-wise pending iter 7.1.x's Bacon-Rajan-style trial-deletion (picks a SAFE cycle edge to weaken). Vector and Hashtable tombstone infrastructure also outstanding as iter 7.1.y. |
