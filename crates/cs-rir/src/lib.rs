...
pub enum Inst {
    ...
    /// `dst = vector->string(v, start, end)` — 3-arg slice form.
    /// ADR 0012 D-2 (iter ID).
    VectorToStringSlice(Value, Value, Value, Value),
    /// `dst = string->vector(s, start, end)` — 3-arg slice form.
    /// ADR 0012 D-2 (iter IE; retry of HN with strict guards).
    StringToVectorSlice(Value, Value, Value, Value),
    ...
    /// `dst = string->list(s, start)` — 2-arg slice-from form.
    /// ADR 0012 D-2 (iter IM).
    StringToListSliceFrom(Value, Value, Value),
    /// `dst = bytevector->list(bv, start)` — 2-arg slice-from form.
    /// ADR 0012 D-2 (iter IN).
    BytevectorToListSliceFrom(Value, Value, Value),
    ...
    /// `dst = vector-copy!(dest, at, src, src_start)` — 4-arg form.
    /// ADR 0012 D-2 (iter IQ).
    VecCopyBangFrom(Value, Value, Value, Value, Value),
    ...
    /// `dst = bytevector-copy!(dest, at, src, src_start, src_end)` — 5-arg.
    /// ADR 0012 D-2 (iter IU).
    BvCopyBangSlice(Value, Value, Value, Value, Value, Value),
    /// `dst = string-copy!(dest, at, src, src_start, src_end)` — 5-arg.
    /// ADR 0012 D-2 (iter IV).
    StrCopyBangSlice(Value, Value, Value, Value, Value, Value),
