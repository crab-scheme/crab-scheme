//! Builtin / special-form table for hover, completion, and signature
//! help (Phase 2 iter 2.5, Phase 4).
//!
//! Single source of truth: `(name, doc)` where `doc` is
//! `"(signature) — description"`. `builtin_doc` looks a name up (hover);
//! `signature` returns just the `(signature)` part (signature help);
//! `NAMES`/`TABLE` enumerate them (completion). Not exhaustive — the
//! common procedures and special forms; extend freely.

/// `(name, "(signature) — description")` for every known builtin and
/// special form.
pub const TABLE: &[(&str, &str)] = &[
    // ---- pairs / lists ----
    (
        "cons",
        "(cons obj1 obj2) — a new pair with car obj1, cdr obj2.",
    ),
    ...
    (
        "list",
        "(list obj …) — a newly allocated list of its arguments.",
    ),
    ...
    (
        "reverse",
        "(reverse list) — list with elements in reverse order.",
    ),
    (
        "list-ref",
        "(list-ref list k) — the kth element (0-based) of list.",
    ),
    (
        "list-tail",
        "(list-tail list k) — list after dropping k elements.",
    ),
    (
        "assoc",
        "(assoc obj alist) — first pair whose car equal? obj, else #f.",
    ),
    ("assq", "(assq obj alist) — like assoc using eq?."),
    (
        "member",
        "(member obj list) — sublist from the first equal? obj, else #f.",
    ),
    ("memq", "(memq obj list) — like member using eq?."),
    (
        "map",
        "(map proc list1 list2 …) — apply proc elementwise, collecting results.",
    ),
    (
        "for-each",
        "(for-each proc list1 …) — apply proc elementwise for effect.",
    ),
    (
        "apply",
        "(apply proc arg … list) — call proc with the args plus list's elements.",
    ),
    // ---- numbers ----
    ...
    (
        "remainder",
        "(remainder n1 n2) — remainder with the sign of n1.",
    ),
    ...
    (
        "eq?",
        "(eq? obj1 obj2) — identity comparison (fast, pointer-ish).",
    ),
    (
        "eqv?",
        "(eqv? obj1 obj2) — like eq? but compares numbers/chars by value.",
    ),
    ...
    (
        "vector",
        "(vector obj …) — a newly allocated vector of its arguments.",
    ),
    (
        "make-vector",
        "(make-vector k [fill]) — a vector of k elements.",
    ),
    ("vector-ref", "(vector-ref vec k) — the kth element of vec."),
    (
        "vector-set!",
        "(vector-set! vec k obj) — store obj at index k.",
    ),
    (
        "vector-length",
        "(vector-length vec) — number of elements in vec.",
    ),
    (
        "string-length",
    ...
    (
        "string-append",
        "(string-append s …) — concatenate the strings.",
    ),
    (
        "substring",
        "(substring s start end) — the substring s[start, end).",
    ),
    // ---- I/O ----
    (
        "display",
        "(display obj [port]) — write obj in human-readable form.",
    ),
    (
        "write",
        "(write obj [port]) — write obj in machine-readable form.",
    ),
    ...
    (
        "letrec",
        "(letrec ((name val)…) body…) — mutually recursive local bindings.",
    ),
    ...
    (
        "case",
        "(case key ((datum…) body…)… [(else body…)]) — dispatch on eqv?.",
    ),
    ("when", "(when test body…) — evaluate body if test is true."),
    (
        "unless",
        "(unless test body…) — evaluate body if test is false.",
    ),
    (
        "begin",
        "(begin expr…) — evaluate in sequence, return the last.",
    ),
    ("quote", "(quote datum) / 'datum — the datum, unevaluated."),
    ("set!", "(set! name value) — assign to an existing binding."),
];

/// Hover doc for `name` (the full "(sig) — description"), or `None`.
pub fn builtin_doc(name: &str) -> Option<&'static str> {
    TABLE.iter().find(|(n, _)| *n == name).map(|(_, d)| *d)
}

/// Just the `(signature)` part of `name`'s doc — the text before " — ".
/// Used by signature help.
pub fn signature(name: &str) -> Option<&'static str> {
    builtin_doc(name).map(|d| d.split(" — ").next().unwrap_or(d))
}
...
mod tests {
    ...
    #[test]
    fn signature_strips_description() {
        assert_eq!(signature("cons"), Some("(cons obj1 obj2)"));
    }

    #[test]
    fn unknown_name_has_no_doc() {
        assert!(builtin_doc("my-helper").is_none());
        assert!(signature("my-helper").is_none());
    }
}
