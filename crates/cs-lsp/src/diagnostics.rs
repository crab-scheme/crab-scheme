//! Source → LSP diagnostics (Phase 1 iters 1.3/1.4).
//!
...
//! match exactly what the compiler would report. Span→range conversion
//! lives in [`crate::text`].
...
pub fn analyze(name: &str, text: &str) -> Vec<Diagnostic> {
    ...
    match read_all(file, text, &mut syms) {
        Err(errors) => errors
            .iter()
            .map(|e| diagnostic(text, e.span(), e.message()))
            .collect(),
        Ok(data) => {
            ...
            match expander.expand_program(&data) {
                Ok(_) => Vec::new(),
                Err(e) => vec![diagnostic(text, e.span(), e.message())],
            }
        }
    }
}

fn diagnostic(text: &str, span: Span, message: String) -> Diagnostic {
    Diagnostic {
