...
// ===== NanboxValue / NB codec (moved to cs-core, cs-vnf.3 PR1) =====
//
// The NaN-box bit-packing primitives, `NanboxValue` itself, and the
// thin-procedure `proc_table` all moved down into
// `cs_core::nanbox` so cs-core can encode/decode NB payloads
// directly (a prerequisite for storing NB payloads in `Pair`'s
// car/cdr slots, PR2). Re-exported here unchanged so every existing
// `cs_vm::vm::{NanboxValue, NB_TAG_PAIR, nb_make, ...}` call site
// (this file, cs-jit-cranelift's lowering, etc.) keeps compiling
// with no source changes.
pub use cs_core::nanbox::{
    nb_decode_gc_ptr, nb_encode_flonum, nb_encode_gc_ptr, nb_is_tagged, nb_make, nb_payload_of,
    nb_sign_extend_47, nb_tag_of, NanboxValue, NB_FIXNUM_MAX, NB_FIXNUM_MIN, NB_NAN_BITS,
    NB_PAYLOAD_MASK, NB_REGION_FLAG, NB_SIGNATURE_BITS, NB_SIGNATURE_MASK, NB_TAG_BOOLEAN,
    NB_TAG_BYTEVECTOR, NB_TAG_CHARACTER, NB_TAG_EOF, NB_TAG_FIXNUM, NB_TAG_GC_VALUE,
    NB_TAG_HASHTABLE, NB_TAG_MASK, NB_TAG_NULL, NB_TAG_PAIR, NB_TAG_PORT, NB_TAG_PROCEDURE,
    NB_TAG_PROMISE, NB_TAG_SHIFT, NB_TAG_STRING, NB_TAG_SYMBOL, NB_TAG_UNSPECIFIED, NB_TAG_VECTOR,
};
// `decode_gc_handle` and `proc_table` were `pub(crate)`/private in
...
/// `ht_raw` and `key_raw` must be live, owned `Gc<Value>` raw handles.
#[no_mangle]
pub unsafe extern "C" fn vm_hashtable_contains_p_gc(ht_raw: i64, key_raw: i64) -> i64 {
    let ht_v = unsafe { gc_i64_to_value(ht_raw) };
    let key_v = unsafe { gc_i64_to_value(key_raw) };
    match ht_v {
        Value::Hashtable(h) if h.eq_kind != cs_core::HtEqKind::Custom => {
            let kind = h.eq_kind;
            let items = h.items.borrow();
            let found = items.iter().any(|(k, _)| match kind {
                cs_core::HtEqKind::Eq => cs_core::eq::eq(k, &key_v),
                cs_core::HtEqKind::Eqv => cs_core::eq::eqv(k, &key_v),
                cs_core::HtEqKind::Equal => cs_core::eq::equal(k, &key_v),
                cs_core::HtEqKind::Custom => unreachable!(),
...
/// `end` are raw Fixnums. ADR 0012 D-2 (iter HC).
///
/// # Safety
///
/// `r` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_copy_slice_gc(r: i64, start: i64, end: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(r) };
    match v {
        Value::ByteVector(bvc) => {
            let storage = bvc.borrow();
            ...
            let slice: Vec<u8> = storage[(start as usize)..(end as usize)].to_vec();
            drop(storage);
            value_to_gc_i64(Value::ByteVector(cs_gc::Gc::new(std::cell::RefCell::new(
                slice,
            ))))
...
/// `(vector-copy v start)` — 2-arg slice form. Slices from `start` to
/// the end of the vector. Consumes `v`. Non-vector or out-of-range
/// (start < 0 or > len) deopt. `start` is a raw Fixnum.
...
/// `r` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_vector_copy_from_gc(r: i64, start: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(r) };
    match v {
        Value::Vector(vc) => {
...
/// `ch`/`start`/`end` are raw Fixnum-shape i64. Non-string, invalid
/// codepoint, or out-of-range (start > end, end > char-count, neg)
/// deopt. ADR 0012 D-2 (iter HH).
///
/// # Safety
///
/// `s` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_string_fill_slice_gc(s: i64, ch: i64, start: i64, end: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(s) };
    let new_ch = match char::from_u32(ch as u32) {
        Some(c) => c,
        None => {
...
/// i64. Non-bytevector or out-of-range deopt to bytecode. ADR 0012
/// D-2 (iter IH).
///
/// # Safety
///
/// `r` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_to_list_slice_gc(r: i64, start: i64, end: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(r) };
    match v {
        Value::ByteVector(b) => {
            let bv = b.borrow();
            ...
            let slice = bv[(start as usize)..(end as usize)]
                .iter()
                .map(|byte| Value::fixnum(*byte as i64));
            value_to_gc_i64(Value::list(slice))
        }
...
/// `r` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_vector_to_list_slice_gc(r: i64, start: i64, end: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(r) };
    match v {
        Value::Vector(vc) => {
            let storage = vc.borrow();
...
/// Native-endian 2-byte ByteVector reads/writes — `u16` and `s16`
/// `native-ref` / `native-set!`. R6RS skips alignment requirements
/// for these (the bytevector indexes by element-aligned bytes per
/// the spec text but the bytecode b_bytevector_*_native_ref accepts
/// any byte offset, matching the macro impl in builtins). ADR 0012
/// D-2 (iter FQ).
///
/// # Safety
///
...
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_u16_native_ref_gc(bv: i64, k: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(bv) };
    match v {
        Value::ByteVector(bvc) => {
            let storage = bvc.borrow();
            if k < 0 || (k as usize).saturating_add(2) > storage.len() {
                jit_request_deopt(DEOPT_REASON_PAIR_MISS);
                return 0;
            }
            let idx = k as usize;
            let buf = [storage[idx], storage[idx + 1]];
            u16::from_ne_bytes(buf) as i64
        }
        _ => {
            jit_request_deopt(DEOPT_REASON_PAIR_MISS);
            0
...
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_s16_native_ref_gc(bv: i64, k: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(bv) };
    match v {
        Value::ByteVector(bvc) => {
            let storage = bvc.borrow();
...
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_u16_native_set_gc(bv: i64, k: i64, val: i64) -> i64 {
...
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_s16_native_set_gc(bv: i64, k: i64, val: i64) -> i64 {
...
/// `(bytevector-copy! dest at src start end)` — 5-arg form: copies
/// `src[start..end]` into `dest` starting at index `at`. Consumes
/// both Gc handles. ADR 0012 D-2 (iter IU).
///
/// # Safety
///
/// `dest` and `src` must be live, owned `Gc<Value>` raw handles.
#[no_mangle]
pub unsafe extern "C" fn vm_bytevector_copy_bang_slice_gc(
    dest: i64,
    at: i64,
    src: i64,
    src_start: i64,
    ...
    let n = end - start;
    {
        ...
        d[at..at + n].copy_from_slice(&src_bytes[start..end]);
    }
    value_to_gc_i64(Value::Unspecified)
}
...
/// `dest` and `src` must be live, owned `Gc<Value>` raw handles.
/// `at` is raw i64.
/// `(string-copy! dest at src start end)` — 5-arg form: copies
/// `src[start..end]` (char-based, multibyte UTF-8 safe) into `dest`
/// starting at index `at`. Consumes both Gc handles. ADR 0012 D-2
/// (iter IV).
///
/// # Safety
///
/// `dest` and `src` must be live, owned `Gc<Value>` raw handles.
#[no_mangle]
pub unsafe extern "C" fn vm_string_copy_bang_slice_gc(
    dest: i64,
    at: i64,
    src: i64,
    src_start: i64,
...
/// `dest` and `src` must be live, owned `Gc<Value>` raw handles.
#[no_mangle]
pub unsafe extern "C" fn vm_vector_copy_bang_from_gc(
    dest: i64,
    at: i64,
    src: i64,
    src_start: i64,
...
/// `(iota count start)` — 2-arg form. Returns
/// `(start start+1 ... start+count-1)` as a fresh list of Fixnums.
/// Both args are raw Fixnum-shape i64. On negative count, requests
/// a deopt and returns Null. ADR 0012 D-2 (iter FC).
///
/// # Safety
///
/// Both args are raw i64; no Gc handle invariants.
#[no_mangle]
pub unsafe extern "C" fn vm_iota_ns_gc(count: i64, start: i64) -> i64 {
    if count < 0 {
        jit_request_deopt(DEOPT_REASON_PAIR_MISS);
        return value_to_gc_i64(Value::Null);
    }
...
/// `s`, `from`, `to` must be live, owned `Gc<Value>` raw handles.
#[no_mangle]
pub unsafe extern "C" fn vm_string_replace_all_gc(s: i64, from: i64, to: i64) -> i64 {
    let s_v = unsafe { gc_i64_to_value(s) };
    let f_v = unsafe { gc_i64_to_value(from) };
    let t_v = unsafe { gc_i64_to_value(to) };
    let (s_str, f_str, t_str) = match (s_v, f_v, t_v) {
...
/// `(string->vector s start)` — 2-arg slice-from form. Returns a
/// fresh `Value::Vector` of chars at codepoint positions `start..`.
/// Char-based indexing (multibyte UTF-8). Consumes `s`. `start` is
/// a raw Fixnum i64. Non-string or out-of-range deopt. ADR 0012
/// D-2 (iter IP).
///
/// # Safety
///
/// `s` must be a live, owned `Gc<Value>` raw handle.
#[no_mangle]
pub unsafe extern "C" fn vm_string_to_vector_slice_from_gc(s: i64, start: i64) -> i64 {
    let v = unsafe { gc_i64_to_value(s) };
    match v {
        Value::String(sg) => {
            ...
            let slice: Vec<Value> = chars[(start as usize)..]
                .iter()
                .copied()
                .map(Value::Character)
                .collect();
...
pub unsafe extern "C" fn vm_closure_id_peek(callee: i64) -> u32 {
    ...
    // Decode the `Gc<Value>` pointer from the payload (shift-
    // decompressed + region flag stripped by `nb_decode_gc_ptr`).
    let (ptr, _is_region) = nb_decode_gc_ptr(nb_payload_of(bits));
...
// =========================================================================
//
// The cs-aot project emitter needs to wrap a Rust `extern "C" fn`
// (an AOT-compiled lambda) as a Scheme `Procedure` value so that
// `MakeClosure` + general `Call` can flow through it. This module
// adds the minimum public surface for that:
//
//   - `VmAotClosure` — Procedure-impl that stores a fn pointer + arity.
//   - `vm_alloc_aot_procedure` — allocates one, returns an NB carrier.
//   - `vm_call_aot_procedure`  — invokes one, given NB-encoded args.
//
// The AOT-emitted dispatch signature is `extern "C" fn(*const i64,
// usize) -> i64` — takes a slice of NB-encoded args + count, returns
// an NB-encoded result. cs-aot generates one of these wrappers per
// AOT'd lambda (iter 2.2's scope; this iter only provides the
// runtime surface).
//
// Capture semantics intentionally NOT included in 2.1 — see iter
// 2.4 plan in aot-hardening-plan.md. This API is sufficient for
...
pub struct VmAotClosure {
    ...
    /// as `usize` so the struct is `Trace`-friendly (no Gc-tracked
    /// pointer to worry about) and stays Send-able if we ever need
    /// cross-thread Procedure handles.
    fn_ptr: usize,
    /// Declared arity (number of positional args). `vm_call_aot_
    /// procedure` validates the caller's arg count against this
    /// before invoking — catches obvious shape mismatches before
    /// the dispatch fn would.
    arity: u32,
...
impl VmAotClosure {
    /// Construct an AOT-procedure wrapper. The `fn_ptr` must be a
    /// valid `AotDispatchFn` address; the caller is responsible
    /// for that invariant. `arity` is the number of positional args
    /// the underlying AOT'd lambda expects.
    ///
    /// # Safety
    ///
    /// Passing an `fn_ptr` that isn't a valid `AotDispatchFn` is UB
    /// (it'll be transmuted + called when the procedure is invoked).
    /// AOT-emitted code always passes a `&fn_name as *const _ as
    /// usize` where `fn_name`'s declared signature matches, so the
    /// invariant holds for AOT call sites.
    pub unsafe fn new(fn_ptr: usize, arity: u32, name: Option<String>) -> Self {
        Self {
            fn_ptr,
            arity,
            name,
...
impl cs_core::Procedure for VmAotClosure {
...
///
/// `disp_fn` is the AOT-emitted dispatch wrapper's function-pointer
/// cast to `usize`. `arity` is the underlying lambda's positional
/// arg count. The returned i64 is an NB-encoded `Procedure` value
/// the caller can pass around like any other Scheme value.
///
/// # Safety
///
/// `disp_fn` must be a valid `AotDispatchFn` address. Caller takes
/// ownership of one strong refcount on the resulting NB carrier
/// (consume-on-use; pass to `vm_value_drop_gc` to release).
#[no_mangle]
...
/// RC3 iter 2.1 — invoke an AOT procedure with the given NB-encoded
/// args. Returns the dispatch fn's result NB-encoded.
///
/// # Safety
///
/// - `proc_nb` must be a valid NB carrier with `NB_TAG_PROCEDURE`
///   referring to a `VmAotClosure` in the proc_table.
/// - `args_ptr` must point at `n_args` valid NB-encoded i64 values.
/// - The dispatch fn the procedure wraps is invoked with these
///   args; it must satisfy the `AotDispatchFn` contract.
///
/// Panics:
/// - The NB carrier's tag isn't NB_TAG_PROCEDURE.
/// - The Procedure isn't a VmAotClosure (e.g., it was a Scheme
///   built-in or a VM-tier VmClosure).
/// - The arg count doesn't match the procedure's declared arity.
#[no_mangle]
pub unsafe extern "C" fn vm_call_aot_procedure(
    proc_nb: i64,
    args_ptr: *const i64,
    n_args: usize,
) -> i64 {
    ...
    assert!(
        ...
        "vm_call_aot_procedure: NB carrier 0x{bits:016x} is not NB_TAG_PROCEDURE"
    );
    let idx = nb_payload_of(bits) as u32;
    let p = proc_table::peek(idx);
    let aot = p
        ...
        .expect("vm_call_aot_procedure: procedure is not a VmAotClosure");
    assert_eq!(
        aot.arity as usize, n_args,
        "vm_call_aot_procedure: arity mismatch — procedure expects {}, got {}",
        aot.arity, n_args
    );
...
mod aot_proc_tests {
    ...
    // for `vm_call_aot_procedure(_, _, wrong_n_args)` was tried but
    // doesn't work — `vm_call_aot_procedure` is `extern "C"`, and
    // panicking across that boundary is undefined behavior in Rust
    // (manifests as a non-unwinding abort here). The arity check is
    // exercised end-to-end via the integration tests once iter 2.3
    // wires it through cs-aot's general-Call emission.
}
...
mod gc_helper_tests {
    ...
    fn vm_alloc_pair_gc_roundtrip() {
        ...
        match v {
            Value::Pair(p) => match (&p.car(), &p.cdr()) {
